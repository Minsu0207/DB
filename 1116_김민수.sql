...
select distinct 과목명
from 과목
inner join 수강신청내역 on 과목.과목번호=수강신청내역.과목번호
inner join 수강신청 on 수강신청내역.수강신청번호=수강신청.수강신청번호
inner join 학생 on 수강신청.학번=학생.학번
...
-- 57 <수강신청내역> 테이블에서 아래와 같이 과목별로 수강자수를 읽어오는(과목번호, 수강자 수) SQL문을 작성하시오.
...
from 수강신청  inner join 학생 on 수강신청.학번 = 학생.학번
where 연도=2018;

-- 66 학번이 1801001'인 학생이 수강 신청한 수강신청번호, 과목번호, 과목명을 출력하는 SQL 문을 작성하시오.
select 수강신청내역.수강신청번호, 과목.과목번호, 과목명
from 수강신청내역 
inner join 과목 on 수강신청내역.과목번호 = 과목.과목번호
inner join 수강신청 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호
...
-- 71 <수강신청내역> 테이블에서 수강신청번호 '1810001'와 '1610001'에 대하여 수강신청번호와 수강 과목번호,수강 과목명 출력하는 SQL 문을 작성하시오
select 수강신청번호,과목.과목번호,과목명
from 수강신청내역 
inner join 과목 on 수강신청내역.과목번호 = 과목.과목번호
where 수강신청번호 IN ('1810001','1610001');

-- 72 모든 학생에 대하여 학번, 학과번호, 이름, 소속 학과 이름을 출력하는 SOL 문을 작성하시오
select 수강신청번호, 학생.학번, 이름
from 수강신청  inner join 학생 on 수강신청.학번 = 학생.학번
where 연도=2018;
...
select 수강신청.학번,이름, count(*) as 과목수, avg(평점) as 평균평점
from 수강신청내역
inner join 수강신청 on 수강신청내역.수강신청번호 = 수강신청.수강신청번호
inner join 학생 on 수강신청.학번 = 학생.학번
where 수강신청내역.평점 >=0
